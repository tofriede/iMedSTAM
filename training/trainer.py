...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        prev_seed_value = get_previous_seed(self.seed_save_path)
        if prev_seed_value is not None:
            self.seed_value = prev_seed_value + 100
        else:
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            # Save the seed value if training is resumed later
            with g_pathmgr.open(self.seed_save_path, "w") as file:
                file.write(str(self.seed_value))
