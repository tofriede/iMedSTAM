...
from template.get_boxes import mask2D_to_bbox
...
def mask_to_box(masks: torch.Tensor):
    ...
    """
    device = masks.device
    box_2d = mask2D_to_bbox(masks[0][0].cpu().numpy())
    x_min, y_min, x_max, y_max = box_2d
    bbox_coords = torch.tensor([[[x_min, y_min, x_max, y_max]]], device=device, dtype=torch.int32)
    return bbox_coords

def mask_to_box_old(masks: torch.Tensor):
    """
    compute bounding box given an input mask
