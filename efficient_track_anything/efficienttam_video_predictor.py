...
class EfficientTAMVideoPredictor(EfficientTAMBase):
    ...
    def init_state(
        ...
        video_height = images_nparray[0].shape[0]
        video_width = images_nparray[0].shape[1]
        images = self._transform_images_input(images_nparray)
        ...
        inference_state["object_boundaries"] = {}
        # Warm up the visual backbone and cache the image feature on frame 0
        self._get_image_feature(inference_state, frame_idx=0, batch_size=1)
        return inference_state
    
    def _transform_images_input(self, images_nparray):
        ...
        images = []
        for i in images_nparray:
            i = Image.fromarray(i).convert("RGB").resize((self.image_size, self.image_size))
            i = np.array(i) / 255
    ...
    def set_images(self, inference_state, images_nparray):
        images = self._transform_images_input(images_nparray)
        inference_state["images"] = images
        inference_state["num_frames"] = len(images)
    ...
    def add_new_points_or_box(
        ...
        obj_boundaries = inference_state["object_boundaries"].get(obj_id, None)
    ...
    def _get_orig_video_res_output(self, inference_state, any_res_masks):
        ...
        if self.non_overlap_masks:
            video_res_masks = self.apply_non_overlapping_constraints(video_res_masks)
    ...
    @torch.inference_mode()
    def propagate_in_video_preflight(self, inference_state, obj_id=None):
    ...
    def propagate_in_video(
        ...
        self.propagate_in_video_preflight(inference_state, obj_id=obj_id)
        ...
        for frame_idx in processing_order:
            ...
            for i in range(batch_size):
                if obj_id is not None:
                    obj_idx = inference_state["obj_id_to_idx"][obj_id]
                    boundaries = inference_state["object_boundaries"].get(obj_id, None)
                else:
                    obj_idx = i
                    boundaries = None
                obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
                ...
                pred_masks_per_obj[i] = pred_masks
                object_score_logits_per_obj[i] = object_score_logits
            ...
            yield frame_idx, obj_ids, video_res_masks, object_score_logits_per_obj
    ...
    def add_object_boundaries(self, obj_id: int, boundaries: tuple[int, int, int]):
        self.inference_state["obj_boundaries"][obj_id] = boundaries