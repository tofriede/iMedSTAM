...
class EfficientTAMBase(torch.nn.Module):
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            selected_cond_outputs, unselected_cond_outputs = select_closest_cond_frames(
                frame_idx, cond_outputs, self.max_cond_frames_in_attn, boundaries, track_in_reverse
            )
            ...
            # print(f"currnt frame: {frame_idx}")
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                for t_diff in range(1, max_obj_ptrs_in_encoder):
                    ...
                    if t < 0 or (num_frames is not None and t >= num_frames):
                        break
                    # print(f"adding obj ptr from frame {t}")
                    out = output_dict["non_cond_frame_outputs"].get(
                        t, unselected_cond_outputs.get(t, None)
                    )
                    if out is not None and False:
    ...
    def _encode_new_memory(
        ...
        if self.non_overlap_masks_for_mem_enc and not self.training:
            ...
            pred_masks_high_res = self.apply_non_overlapping_constraints(
                pred_masks_high_res
            )
    ...
    def apply_non_overlapping_constraints(self, pred_masks):
        """
        ...
        pred_masks = torch.where(keep, pred_masks, torch.zeros(pred_masks.shape, device=device))
        return pred_masks
